...
**Two processes, one shared state on disk.** `run.py` spawns the user chat (`app.py`) and admin (`admin.py`) as *separate* Streamlit processes. They cannot share memory, so cross-process state lives on disk:
